...
import { Handle, Position, type NodeProps, useConnection, useUpdateNodeInternals } from '@xyflow/react';
...
function ShapeNodeInner({ id, data, selected }: NodeProps<ShapeNodeType>) {
  ...
  const getSourceStyle = (handleId: string) => {
    const isActive = isConnecting && activeNodeId === id && activeHandleId === handleId;
    return { pointerEvents: isConnecting ? (isActive ? 'auto' : 'none') : 'auto' } as React.CSSProperties;
  };
  ...
  return (
    <div className={`shape-node-container ${selected ? 'shape-selected' : ''}`}>
      <div className="shape-svg-wrapper" style={{ width: SHAPE_SIZE, height: SHAPE_SIZE }}>
        <svg
          width={SHAPE_SIZE}
          height={SHAPE_SIZE}
        ...
        {/* Handles at the 4 cardinal points inside the shape wrapper so they touch the bounds of SHAPE_SIZE */}
      ...
      {data.label && (
        <div className="shape-node-label" style={{ color: bwMode ? '#000' : '#fff' }}>
          {data.label}
        </div>
      )}
    </div>
